...
<head>
	<base href="<%= request.getScheme() + "://" + request.getServerName() + ":" + request.getServerPort() + request.getContextPath() %>/" />
  	...
  	<title>主机性能详情 - 性能查询 - 云主机性能监测</title>
			...
			<div class="panel panel-default front-panel">
				<div class="panel-heading">
      				<h3 class="panel-title">     				
         				<a href="search/vmdetail" style="float: left; color: #337ab7" >性能查询&nbsp;</a>
						<span class="active" id="index" ></span>
      				</h3>
   				</div>
   				<div class="panel-body" style="background: #fafafa;">
						...
						<div class="front-rw-right-btns">
							...
							<button type="button" id="groupsearch" class="btn btn-primary" onclick="compareConfirm(${param.id},${param.name},${param.cpu},${param.ram})">确定</button>
						</div>
					</div>		
   				</div>
			</div>			
			<div class="btn-group" style="margin-bottom: 20px;">
				<a class="btn btn-default" onclick="exportVmExcel()">导出数据</a>
				<a class="btn btn-default" onclick="addCompare(this)" id="addrr" data-id="" data-cpu="" data-mem="">加入对比</a>
			</div>
...
<%-- <script type="text/javascript">
          window.onload = function() {
              $LAB
           .script("http://code.jquery.com/jquery.js").script("js/bootstrap.min.j").script("js/timeview.js").script("js/init_storage.js");
          }
</script> --%>
...
$(function(){
	...
	var cpu = ${param.cpu};
	var ram = ${param.ram};
	compareConfirm(id,name,cpu,ram);
	var indexname = name.replace("'","");
	$("#index").text(" / "+indexname+" [4核8G]");
	$("#addrr").attr("data-id",id);
	$("#addrr").attr("data-cpu",cpu);
	$("#addrr").attr("data-mem",ram);
});
function resetting(){
